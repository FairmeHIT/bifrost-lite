...
def read_denylist():
    """Return denied paths.  Plain entries match exactly or as directories;
    entries ending in `*` match as explicit prefixes."""
    entries = []
...
def cmd_denylist(args):
    ...
    for stem in STEMS:
        prefix = stem[:-1]
        scattered = [f for f in scattered if not f.startswith(prefix)]
    ...
    lines = [
        ...
        "# enterprise-exclusive / lite-deleted features.  Skip rule: a path",
        "# matches an exact entry, a child of a directory entry, or an explicit",
        "# wildcard prefix entry ending in `*`.",
        "",
