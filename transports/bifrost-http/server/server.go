...
// GetMCPClientsAllowingAllVirtualKeys is required by the governance plugin's
// InMemoryStore interface. MCP is removed, so this always returns an empty map.
...
// The plugin is registered to the appropriate arrays based on which interfaces it implements.
...
// The plugin is removed from the LLM array if it exists in it.
...
func (s *BifrostHTTPServer) Bootstrap(ctx context.Context) error {
	...
	if s.Config.ConfigStore == nil {
		...
		// browser-only" workflow plugs into
