...
func loadMCPConfig(ctx context.Context, config *Config, configData *ConfigData) {
	...
	if configData.MCP != nil && len(configData.MCP.ClientConfigs) > 0 {
		...
		for _, c := range configData.MCP.ClientConfigs {
			...
			if err := validateMCPClientName(c.Name); err != nil {
...
func applyMCPClientPinnedStateToRow(row *configstoreTables.TableMCPClient, clientConfig *schemas.MCPClientConfig) {
	...
	row.PerUserHeaderKeys = canonicalizeHeaderKeys(clientConfig.PerUserHeaderKeys)
...
func mcpClientConfigToTable(clientConfig *schemas.MCPClientConfig) (configstoreTables.TableMCPClient, error) {
	...
	return configstoreTables.TableMCPClient{
		...
		PerUserHeaderKeys:         canonicalizeHeaderKeys(clientConfig.PerUserHeaderKeys),
...
// validateMCPClientName validates an MCP client name. Kept inline from the
// removed core/mcp package so legacy MCP config sections still validate.
...
// canonicalizeHeaderKeys returns a new slice with every entry lowercased and
// trimmed. Nil in → nil out so "nil means preserve existing" semantics hold.
