...
export function AddCustomProviderSheetContent({ show = true, onClose, onSave }: AddCustomProviderSheetContentProps) {
	...
	const onSubmit = (data: FormData) => {
		...
		addProvider(payload)
			...
			.catch((err) => {
				toast.error(t("providers.addCustomProvider.failed"), {
				...
				<form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col">
					<div className="min-h-0 flex-1 space-y-4 px-8 pb-4">
						<FormField
							...
							render={({ field }) => (
								<FormItem className="flex flex-col gap-3">
									<FormLabel className="text-right">{t("common.name")}</FormLabel>
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem className="flex flex-col gap-3">
									<FormLabel>{t("providers.addCustomProvider.baseFormat")}</FormLabel>
									<div>
										<FormControl>
											<Select onValueChange={field.onChange} value={field.value} disabled={!hasProviderCreateAccess}>
												<SelectTrigger className="w-full" data-testid="base-provider-select">
													<SelectValue placeholder={t("providers.addCustomProvider.selectBaseFormat")} />
												</SelectTrigger>
												<SelectContent>
													<SelectItem value="openai">{t("providers.addCustomProvider.baseFormatOpenAI")}</SelectItem>
													<SelectItem value="anthropic">{t("providers.addCustomProvider.baseFormatAnthropic")}</SelectItem>
													<SelectItem value="gemini">{t("providers.addCustomProvider.baseFormatGemini")}</SelectItem>
													<SelectItem value="cohere">{t("providers.addCustomProvider.baseFormatCohere")}</SelectItem>
													<SelectItem value="bedrock">{t("providers.addCustomProvider.baseFormatBedrock")}</SelectItem>
													<SelectItem value="replicate">{t("providers.addCustomProvider.baseFormatReplicate")}</SelectItem>
												</SelectContent>
											</Select>
										</FormControl>
										<FormMessage />
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem className="flex flex-col gap-3">
									<FormLabel>{t("providers.addCustomProvider.baseUrl")}</FormLabel>
									<div>
										<FormControl>
											<Input
												placeholder={t("providers.addCustomProvider.baseUrlPlaceholder")}
									...
									<div className="flex items-center justify-between space-x-2 rounded-lg border p-3">
										<div className="space-y-0.5">
											<label htmlFor="allow-private-network" className="text-sm font-medium">
												{t("providers.addCustomProvider.allowPrivateNetwork")}
											</label>
											<p className="text-muted-foreground text-sm">{t("providers.addCustomProvider.allowPrivateNetworkDesc")}</p>
										</div>
										<Switch
										...
										<div className="flex items-center justify-between space-x-2 rounded-lg border p-3">
											<div className="space-y-0.5">
												<label htmlFor="drop-excess-requests" className="text-sm font-medium">
													{t("providers.addCustomProvider.isKeyless")}
												</label>
												<p className="text-muted-foreground text-sm">{t("providers.addCustomProvider.isKeylessDesc")}</p>
											</div>
											<Switch
					...
					<div className="bg-card sticky bottom-0 ml-auto flex w-full flex-row gap-2 border-t px-8 py-4">
						...
						<Button type="submit" isLoading={isAddingProvider} disabled={!hasProviderCreateAccess} data-testid="custom-provider-save-btn">
							{t("providers.addCustomProvider.submit")}
						</Button>
					</div>
				</form>
			</Form>
