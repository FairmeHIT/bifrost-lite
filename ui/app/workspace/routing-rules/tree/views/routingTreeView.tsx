	...
	if (isError) {
		return (
			<div className="text-muted-foreground flex h-full items-center justify-center gap-2">
				...
				<span className="text-sm">{t("routingRules.tree.loadFailed")}</span>
			</div>
		);
	}
	if (rules.length === 0) {
		return (
			<div className="text-muted-foreground flex h-full flex-col items-center justify-center gap-3">
				...
				<p className="text-sm">{t("routingRules.tree.noRules")}</p>
				<Button
					variant="outline"
					size="sm"
							...
							<p className="text-muted-foreground text-[11px]">
								{search
									? highlightedIds && highlightedIds.size > 0
										? t("routingRules.tree.matchCount", { count: matchCount })
					...
					<div className="dark:bg-card flex items-center gap-3 rounded-md border bg-white px-3 py-1.5 shadow-sm">
						{SCOPE_ORDER.map((s) => (
							<div key={s} className="flex items-center gap-1.5">
								...
								<span className="text-muted-foreground text-[10px] font-medium">{t(SCOPE_LABEL_KEYS[s])}</span>
							</div>
						))}
						...
						<div className="flex items-center gap-1.5">
							...
							<Tooltip>
								...
								<TooltipContent side="top" className="max-w-[200px] text-center">
									{t("routingRules.tree.staticChainDesc")}
								</TooltipContent>
							</Tooltip>
						</div>
						<div className="flex items-center gap-1.5">
							...
							<Tooltip>
								...
								<TooltipContent side="top" className="max-w-[200px] text-center">
									{t("routingRules.tree.dynamicChainDesc")}
								</TooltipContent>
							</Tooltip>
						</div>
					</div>
