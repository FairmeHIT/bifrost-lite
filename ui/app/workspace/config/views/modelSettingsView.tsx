...
export default function ModelSettingsView() {
	...
	const onSubmit = async (data: ModelSettingsFormData) => {
		try {
			...
			toast.success(t("modelSettings.updated"));
			reset(data);
		} catch (error) {
	...
	const handleForceSync = async () => {
		try {
			...
			toast.success(t("modelSettings.pricingSynced"));
		} catch (error) {
			toast.error(getErrorMessage(error));
		}
	};
		...
		<div className="mx-auto w-full max-w-7xl space-y-4" data-testid="model-settings-view">
			<form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
				...
				<div className="space-y-4">
					...
					<div className="space-y-2 rounded-sm border p-4">
						<div className="space-y-0.5">
							<Label htmlFor="pricing-datasheet-url">{t("modelSettings.pricingDatasheetUrl")}</Label>
							<p className="text-muted-foreground text-sm">{t("modelSettings.pricingDatasheetUrlDescription")}</p>
						</div>
						<Input
					...
					<div className="space-y-2 rounded-sm border p-4">
						<div className="space-y-0.5">
							<Label htmlFor="model-parameters-url">{t("modelSettings.modelParametersUrl")}</Label>
							<p className="text-muted-foreground text-sm">{t("modelSettings.modelParametersUrlDescription")}</p>
						</div>
						<Input
					...
					<div className="space-y-2 rounded-sm border p-4">
						<div className="space-y-0.5">
							...
							<p className="text-muted-foreground text-sm">{t("modelSettings.pricingSyncIntervalDescription")}</p>
						</div>
						<Input
							...
							{...register("pricing_sync_interval_hours", {
								required: t("modelSettings.pricingSyncIntervalRequired"),
								min: { value: 1, message: t("modelSettings.pricingSyncIntervalMin") },
								max: { value: 8760, message: t("modelSettings.pricingSyncIntervalMax") },
								valueAsNumber: true,
							})}
						/>
					...
					<div className="space-y-2 rounded-sm border p-4">
						...
						<Input
							...
							{...register("live_models_sync_interval_minutes", {
								required: t("modelSettings.modelDiscoveryIntervalRequired"),
								validate: (value) => {
									if (value === 0) return true;
									if (value < MIN_LIVE_MODELS_SYNC_INTERVAL / SECONDS_PER_MINUTE) {
										return t("modelSettings.modelDiscoveryIntervalMin", {
											minutes: MIN_LIVE_MODELS_SYNC_INTERVAL / SECONDS_PER_MINUTE,
										});
									}
									if (value > 1440) return t("modelSettings.modelDiscoveryIntervalMax");
									return true;
								},
								valueAsNumber: true,
							})}
					...
					<div className="flex items-center justify-between rounded-sm border p-4">
						...
						<Input
							...
							{...register("routing_chain_max_depth", {
								required: t("modelSettings.routingChainMaxDepthRequired"),
								min: { value: 1, message: t("modelSettings.routingChainMin") },
								max: { value: 100, message: t("modelSettings.routingChainMax") },
								valueAsNumber: true,
							})}
						/>
					</div>
				...
				<div className="flex justify-end gap-2 pt-2">
					...
					>
						{isForceSyncing ? t("modelSettings.syncing") : t("modelSettings.forceSync")}
					</Button>
					<Button type="submit" disabled={!hasChanges || isLoading || !hasSettingsUpdateAccess} data-testid="model-settings-save-btn">
						{isLoading ? t("common.saving") : t("modelSettings.saveChanges")}
					</Button>
				</div>
			</form>
		</div>
