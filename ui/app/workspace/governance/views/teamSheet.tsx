...
export default function TeamSheet({ team, onSave, onCancel }: TeamSheetProps) {
	...
	const validator = useMemo(() => {
		...
		const budgetValidators = formData.budgets.flatMap((row, idx) => {
			...
			return [
				Validator.minValue(row.maxLimit, 0.01, t("governance.budgetNumberMaxLimit", { n: idx + 1 })),
				Validator.required(row.resetDuration, t("governance.budgetNumberResetRequired", { n: idx + 1 })),
			];
		});
		...
		return new Validator([
			...
			...budgetValidators,
			Validator.custom(uniqueDurations === populatedDurations.length, t("teams.distinctResetDuration")),
				...
				<form onSubmit={handleSubmit} className="flex h-full flex-col gap-6">
					<div className="grow space-y-6 px-8">
						...
						<div className="flex flex-col gap-6">
							...
							<div className="space-y-2">
								<Label htmlFor="customer">{t("teams.customerOptional")}</Label>
								...
								<p className="text-muted-foreground text-sm">{t("teams.customerHint")}</p>
							</div>
						</div>
						...
						<div className="space-y-3">
							<div className="flex items-center justify-between">
								<Label>{t("governance.budgets")}</Label>
								<button
									type="button"
									...
									<div className="flex items-start gap-2">
										<div className="flex-1">
											<NumberAndSelect
												...
												label={t("governance.budgetNumberMaxSpend", { n: idx + 1 })}
												value={row.maxLimit}
						...
						<AlertDialog open={showCalendarAlignWarning} onOpenChange={setShowCalendarAlignWarning}>
							<AlertDialogContent>
								...
								<AlertDialogFooter>
									<AlertDialogCancel data-testid="team-calendar-align-cancel-btn">{t("common.cancel")}</AlertDialogCancel>
									<AlertDialogAction
						...
						<BudgetUsageResetDialog
							...
							ownerLabel={t("teams.entityLabel")}
						...
						{isEditing && ((team?.budgets && team.budgets.length > 0) || team?.rate_limit) && (
							<div className="bg-muted/50 space-y-4 rounded-lg border p-4">
								<p className="text-sm font-medium">{t("governance.currentUsage")}</p>
								<div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
									{team?.budgets?.map((b) => (
										<div key={b.id} className="space-y-1">
											<p className="text-muted-foreground text-xs">{t("governance.budgetWithDuration", { duration: b.reset_duration })}</p>
											...
											<p className="text-muted-foreground text-xs">
												{t("governance.lastResetWithTime", {
									...
									{team?.rate_limit?.token_max_limit && (
										<div className="space-y-1">
											<p className="text-muted-foreground text-xs">{t("governance.tokens")}</p>
											<div className="flex items-center gap-2">
												<span className="font-mono text-sm">
									...
									{team?.rate_limit?.request_max_limit && (
										<div className="space-y-1">
											<p className="text-muted-foreground text-xs">{t("governance.requests")}</p>
											<div className="flex items-center gap-2">
												<span className="font-mono text-sm">
									...
									<TooltipTrigger asChild>
										<span className="inline-block">
											<Button type="submit" disabled={loading || !validator.isValid() || !hasPermission} data-testid="team-save-btn">
												{loading ? t("common.saving") : isEditing ? t("teams.editTitle") : t("teams.createTitle")}
											</Button>
										</span>
									</TooltipTrigger>
										...
										<TooltipContent>
											<p>
												{!hasPermission
													...
													: loading
														? t("common.saving")
														: validator.getFirstError() || t("governance.fixValidationErrors")}
											</p>
										</TooltipContent>
									)}
								</Tooltip>
