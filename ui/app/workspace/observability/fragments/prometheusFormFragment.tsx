...
export function PrometheusFormFragment({
	...
	const renderActions = (tabKey: "pull" | "push", tabDirty: boolean, onResetTab: () => void) => {
		...
		const otherTabLabel = tabKey === "pull" ? t("observability.pushBased") : t("observability.pullBased");
		const saveDisabled = !hasPrometheusAccess || !tabDirty || formIsInvalid;
		let tooltipMsg = "";
		if (!tabDirty) {
			tooltipMsg = t("observability.noChangesInTab");
		} else if (formIsInvalid && otherTabHasErrors && !thisTabHasErrors) {
			tooltipMsg = t("observability.fixValidationErrorsInTab", { tab: otherTabLabel });
		} else if (formIsInvalid) {
			tooltipMsg = t("observability.fixValidationErrors");
		}
						...
						<Tooltip>
							<TooltipTrigger asChild>
								<Button type="submit" disabled={saveDisabled} isLoading={isSaving} data-testid={`prometheus-${tabKey}-save-btn`}>
									{t("observability.savePrometheusConfig")}
								</Button>
							</TooltipTrigger>
							{tooltipMsg && (
								<TooltipContent>
				...
				<Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "pull" | "push")}>
					...
					<TabsContent value="pull" className="mt-2 space-y-4">
						<div className="flex items-center justify-between gap-4">
							<div className="flex flex-col gap-1">
								<h3 className="text-sm font-medium">{t("observability.pullBasedScraping")}</h3>
								<p className="text-muted-foreground text-xs">{t("observability.pullBasedDesc")}</p>
							</div>
							<FormField
						...
						<div className="bg-muted/50 rounded-md p-4">
							<div className="flex items-center justify-between">
								<div className="flex flex-col gap-1">
									<span className="text-sm font-medium">{t("observability.metricsEndpoint")}</span>
								...
								{metricsEndpoint && (
									...
									>
										...
										{copied ? t("observability.copied") : t("observability.copyEndpoint")}
									</Button>
								)}
							</div>
							<p className="text-muted-foreground mt-2 text-xs">{t("observability.pullBasedScrapeDesc")}</p>
						</div>
					...
					<TabsContent value="push" className="mt-2 space-y-4">
						<div className="flex items-center justify-between gap-4">
							<div className="flex flex-col gap-1">
								<h3 className="flex flex-row items-center gap-2 text-sm font-medium">
									{t("observability.pushBasedGateway")} <Badge variant="secondary">BETA</Badge>
								</h3>
								<p className="text-muted-foreground text-xs">{t("observability.pushBasedDesc")}</p>
							</div>
							<FormField
								control={form.control}
						...
						<Alert variant="info">
							...
							<AlertDescription className="text-xs">{t("observability.pushGatewayAlert")}</AlertDescription>
						</Alert>
						...
						<div className="space-y-4">
							<FormField
								...
								render={({ field }) => (
									<FormItem className="w-full">
										<FormLabel>{t("observability.pushGatewayUrl")}</FormLabel>
										<FormControl>
											<SecretVarInput
												placeholder={t("prometheusForm.pushGatewayUrlPlaceholder")}
												disabled={!hasPrometheusAccess}
												data-testid="prometheus-push-gateway-url"
												{...field}
											/>
										</FormControl>
										<FormDescription>{t("observability.pushGatewayUrlHelp")}</FormDescription>
										<FormMessage />
									</FormItem>
								)}
							/>
							...
							<div className="grid grid-cols-2 gap-4">
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											<FormLabel>{t("observability.jobName")}</FormLabel>
											<FormControl>
												<Input placeholder="bifrost" disabled={!hasPrometheusAccess} data-testid="prometheus-job-name" {...field} />
											</FormControl>
											<FormDescription>{t("observability.jobNameHelp")}</FormDescription>
											<FormMessage />
										</FormItem>
									)}
								/>
								...
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											...
											<FormDescription>{t("observability.pushIntervalHelp")}</FormDescription>
											<FormMessage />
										</FormItem>
									)}
								/>
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										<FormLabel className="flex items-center gap-2">
											...
											<TooltipProvider>
												<Tooltip>
													...
													<TooltipContent>
														<p className="max-w-xs text-xs">{t("observability.instanceIdHelp")}</p>
													</TooltipContent>
												</Tooltip>
											</TooltipProvider>
										</FormLabel>
										<FormControl>
											<Input
												placeholder={t("observability.instanceIdPlaceholder")}
												disabled={!hasPrometheusAccess}
							...
							<div className="space-y-4 border-t pt-4">
								{!showBasicAuth ? (
									...
									>
										...
										{t("observability.addBasicAuth")}
									</Button>
								) : (
									<>
										<div className="flex items-center justify-between">
											<span className="text-sm font-medium">{t("observability.basicAuthentication")}</span>
											<Button
												type="button"
												variant="ghost"
												size="sm"
												...
												aria-label={t("prometheusForm.removeBasicAuth")}
											>
										...
										<div className="border-muted grid grid-cols-2 gap-4">
											<FormField
												...
												render={({ field }) => (
													<FormItem>
														<FormLabel>{t("observability.username")}</FormLabel>
														<FormControl>
															<SecretVarInput
																placeholder={t("prometheusForm.usernamePlaceholder")}
																disabled={!hasPrometheusAccess}
											...
											<FormField
												...
												render={({ field }) => (
													<FormItem>
														<FormLabel>{t("observability.password")}</FormLabel>
														<FormControl>
															<SecretVarInput
																type="password"
																placeholder={t("prometheusForm.passwordPlaceholder")}
																disabled={!hasPrometheusAccess}
																hideValueWhenEnv
																redactNonEnvValue
