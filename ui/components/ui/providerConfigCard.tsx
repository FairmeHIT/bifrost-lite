...
function WeightInput({
	...
	return (
		<input
			...
			data-testid={testId}
			value={display}
			placeholder={t("providerConfigCard.excludeFromRouting")}
			onChange={(e) => {
...
export function ProviderConfigCard({
	...
	const keysSummary = value.keyIds.includes("*")
		...
		: value.keyIds.length > 0
			? t("providerConfigCard.keyCount", { count: value.keyIds.length })
			: t("providerConfigCard.noKeys");
	const modelsSummary = value.allowedModels.includes("*")
		? t("virtualKeys.allModels")
		: value.allowedModels.length > 0
			? t("providerConfigCard.modelsCount", { count: value.allowedModels.length })
			: t("providerConfigCard.denyAll");
	const hasRl = value.rateLimit?.token_max_limit != null || value.rateLimit?.request_max_limit != null;
	const rlSummary = hasRl ? t("providerConfigCard.rateLimitsSet") : t("providerConfigCard.noRateLimits");
		...
		<div className="bg-card overflow-hidden rounded-md border">
			...
			>
				...
				<button
					...
					aria-label={t("providerConfigCard.removeProvider", { name: providerLabel })}
			...
			{open && (
				<>
					...
					>
						...
						<span className="text-muted-foreground text-sm font-medium whitespace-nowrap">{t("virtualKeys.providerBudget")}</span>
						...
						{ws != null && (
							<span className="text-muted-foreground/70 shrink-0 text-sm whitespace-nowrap">
								{t("providerConfigCard.globalProviderCap", { amount: money(ws.max_limit) })}
					...
					{budgetOpen && (
						<div className="bg-muted/30 border-t py-3 pr-3.5 pl-16">
							<MultiBudgetLines
								...
								label={t("providerConfigCard.providerBudgetLabel")}
					...
					{showModelBudgets && (
						<>
							...
							>
								<span className="text-muted-foreground/50 text-sm">└</span>
								<span className="text-muted-foreground text-sm font-medium whitespace-nowrap">{t("virtualKeys.modelBudgets")}</span>
								<span className="text-muted-foreground/60 min-w-0 flex-1 truncate text-sm">
									{modelsOpen
										? t("providerConfigCard.skipModelsSharedBudget")
										: t("providerConfigCard.modelBudgetCount", { count: modelBudgets.length })}
								</span>
								<ChevronDown
									className={cn(
										...
										<div key={mb.model_name}>
											...
											>
												...
												<span className="text-foreground/90 min-w-0 flex-1 truncate text-sm font-medium">
													{mb.model_name || t("providerConfigCard.selectModel")}
												</span>
												<span className="text-muted-foreground shrink-0 text-sm whitespace-nowrap">
													{budgetLinesLabel(mb.budgets, t("providerConfigCard.noCap"))}
												</span>
												<button
													type="button"
													onClick={(e) => {
													...
													aria-label={t("providerConfigCard.removeModelBudget", { name: mb.model_name || "" })}
													className="text-muted-foreground shrink-0 cursor-pointer rounded-sm p-1 hover:text-red-400"
												>
													<Trash2 className="h-3 w-3" />
												</button>
											...
											{mbEditorOpen && (
												<div className="bg-muted/30 space-y-4 border-t py-3 pr-3.5 pl-16">
													<MultiBudgetLines
														...
														label={t("providerConfigCard.modelBudgetLabel")}
													...
													<NumberAndSelect
														...
														placeholder={t("providerConfigCard.noLimit")}
								...
								<div className="pt-1 pr-3.5 pb-2.5 pl-16">
									<div className="w-[200px]">
										<ModelMultiselect
											...
											placeholder={t("providerConfigCard.addModel")}
										/>
									</div>
								</div>
							)}
					...
					>
						<span className="text-muted-foreground/50 text-sm">└</span>
						<span className="text-muted-foreground text-sm font-medium whitespace-nowrap">{t("providerConfigCard.accessRateLimits")}</span>
						<span className="text-muted-foreground/60 min-w-0 flex-1 truncate text-sm">
					...
					{accessOpen && (
						<div className="bg-muted/20 flex flex-col gap-3 border-t py-3 pr-3.5 pl-10">
							<div className="flex items-start gap-3.5">
								...
								{(() => {
									...
									const allKeyOptions: KeyOption[] = [
										{ label: t("providerConfigCard.allowAllKeys"), value: "*", description: t("providerConfigCard.allowAllKeysDesc") },
										...keys.map((key) => ({
											label: key.name,
											value: key.key_id,
											description:
												key.models == null || key.models.includes("*")
													? t("virtualKeys.allModels")
													: key.models.filter((m) => m !== "*").join(", ") || t("virtualKeys.noModelsDenyAll"),
										})),
									];
												...
												views={{
													multiValue: (multiValueProps: MultiValueProps<KeyOption>) => (
														...
														>
															...
															<button
																type="button"
																aria-label={t("providerConfigCard.remove", { label: multiValueProps.data.label })}
								...
								{(() => {
									...
									return (
										<div className="min-w-0 flex-1 space-y-1.5">
											<div className="flex h-5 items-center">
												<Label>{t("virtualKeys.allowedModels")}</Label>
											</div>
											<ModelMultiselect
												allowAllOption
												hideSearchIcon
												...
												placeholder={
													hasWildcardModels
														? t("providerConfigCard.allModelsAllowed")
														: value.allowedModels.length === 0
															? t("virtualKeys.noModelsDenyAll")
															: t("providerConfigCard.addModel")
												}
											/>
										</div>
									);
							...
							<div className="flex items-start gap-3.5">
								...
								<div className="min-w-0 flex-1 space-y-1.5">
									<div className="flex h-5 items-center">
										<Label>{t("virtualKeys.blockedModels")}</Label>
									</div>
									{(() => {
												...
												placeholder={
													hasWildcardBlocked
														...
														: value.blacklistedModels.length === 0
															? t("providerConfigCard.noBlockedModels")
															: t("providerConfigCard.searchModels")
												}
											/>
										);
									})()}
