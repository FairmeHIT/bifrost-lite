	...
	({ className, type, showCopyButton = false, inputClassName, ...props }, ref) => {
		...
		if (showCopyButton) {
			return (
				<div
					className={cn(
						"dark:bg-input/20 border-input focus-visible:border-primary chamfer-sm flex w-full items-center border bg-transparent",
						className,
					)}
				>
					<input
		...
		return (
			<input
				...
				data-slot="input"
				className={cn(
					"file:text-foreground placeholder:text-muted-foreground/70 selection:bg-primary selection:text-primary-foreground dark:bg-input/20 border-input focus-visible:border-primary chamfer-sm flex h-9 w-full min-w-0 border bg-transparent px-3 py-1 text-base shadow-none transition-[color,box-shadow] outline-none file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm",
					"aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
					className,
					inputClassName,
				)}
