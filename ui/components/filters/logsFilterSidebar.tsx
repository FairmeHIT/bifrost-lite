	...
	if (collapsed) {
		return (
			<button
				...
				title={t("filter.showFilters")}
				aria-label={t("filter.showFilters")}
			>
				<PanelLeftOpen className="text-muted-foreground group-hover:text-foreground size-4 transition-colors" />
				<span className="rotate-180 select-none [writing-mode:vertical-rl]">{t("filter.filters")}</span>
				{activeFilterCount > 0 && (
	...
	return (
		<div className="bg-card flex h-full w-64 shrink-0 flex-col rounded-r-md">
			...
			<div className="flex h-11 items-center justify-between border-b pr-2 pl-5">
				<span className="text-sm font-semibold">{t("filter.filters")}</span>
				<div className="flex items-center gap-1">
					...
					<Button
						...
						className="size-7"
						onClick={toggleCollapsed}
						title={t("filter.hideFilters")}
						aria-label={t("filter.hideFilters")}
					>
...
function StatusFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	const { t } = useI18n();
	return (
		<FilterSection title={t("filter.status")} defaultOpen={defaultOpen || hasActive} testId="status-filter-toggle">
			{Statuses.map((status) => (
				<CheckboxFilterItem
					key={status}
	...
	return (
		...
		>
			<SearchableCheckboxList
				...
				placeholder={t("filter.addStopReason")}
				items={items}
				allowCustom
...
function TypeFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		<FilterSection title={t("filter.type")} defaultOpen={defaultOpen || hasActive} testId="type-filter-toggle">
...
function SelectedKeysFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		...
		>
			<SearchableCheckboxList
				inputRef={searchInputRef}
				placeholder={t("filter.searchKeys")}
...
function VirtualKeysFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		...
		>
			<SearchableCheckboxList
				inputRef={searchInputRef}
				placeholder={t("filter.searchVirtualKeys")}
...
function RoutingEnginesFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		...
		>
			<SearchableCheckboxList
				inputRef={searchInputRef}
				placeholder={t("filter.searchEngines")}
...
function RoutingRulesFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		...
		>
			<SearchableCheckboxList
				inputRef={searchInputRef}
				placeholder={t("filter.searchRules")}
...
function SessionFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		<FilterSection title={t("filter.session")} defaultOpen={defaultOpen || hasActive} testId="session-filter-toggle">
			<div className="relative">
				<Search className="text-muted-foreground pointer-events-none absolute top-1/2 left-2.5 size-3.5 -translate-y-1/2" />
				<Input
					value={filters.parent_request_id || ""}
					onChange={(e) => onFiltersChange({ ...filters, parent_request_id: e.target.value })}
					placeholder={t("filter.parentRequestId")}
...
function CustomerFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		...
		>
			<SearchableCheckboxList
				inputRef={searchInputRef}
				placeholder={t("filter.addCustomer")}
...
function BusinessUnitFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		...
		>
			<SearchableCheckboxList
				inputRef={searchInputRef}
				placeholder={t("filter.addBusinessUnit")}
...
function CostFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		<FilterSection title={t("filter.cost")} defaultOpen={defaultOpen || hasActive} testId="cost-filter-toggle">
			<CheckboxFilterItem
				label={t("filter.showMissingCostOnly")}
				...
				<>
					<div className="relative border-b">
						...
						<Input
							...
							placeholder={t("filter.metadataPlaceholder")}
							className="h-8 border-0 pl-8 text-xs"
						...
						<div key={metadataKey} data-testid={`metadata-${metadataKey}-filter-group`}>
							...
							<div className="px-3 py-2.5">
								<Input
									...
									placeholder={t("filter.customValue")}
									value={
